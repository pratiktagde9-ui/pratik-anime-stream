...
function retrySection(containerId) {
  alreadyLoaded.delete(containerId); // allow it to run again
  enqueueLoad(containerId);
}
...
// Top Rated -> default top anime ranking (lighter query than filter=favorite)
async function loadTopRated() {
  renderSkeletons("toprated", 8);
  const results = await jikanFetchMany("/top/anime", 25);
  renderRow("toprated", results);
}

// Popular Movies -> search endpoint filtered to movies, sorted by score
// (lighter than /top/anime?type=movie which was timing out)
async function loadMovies() {
  renderSkeletons("movies", 8);
  const results = await jikanFetchMany("/anime?type=movie&order_by=score&sort=desc", 25);
  renderRow("movies", results);
}

// Coming Soon -> a specific upcoming season instead of the dynamic
// "/seasons/upcoming" alias, which was timing out server-side
async function loadUpcoming() {
  renderSkeletons("upcoming", 8);
  const results = await jikanFetchMany("/seasons/2026/fall", 25);
...
/* ---------- Init: lazy-load each row only when scrolled into view ---------- */
...
async function processQueue() {
  ...
  while (loadQueue.length > 0) {
    const loaderFn = loadQueue.shift();
    await loaderFn();
    await delay(900); // space out requests even if several sections became visible at once
  }
  queueRunning = false;
}

function enqueueLoad(id) {
  if (alreadyLoaded.has(id) || !sectionLoaders[id]) return;
  alreadyLoaded.add(id);
  loadQueue.push(sectionLoaders[id]);
  processQueue();
}
...
function setupLazyLoad() {
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) enqueueLoad(entry.target.id);
    });
